var customerList = {
    ...
    init: function () {
        ...
        $("#search-customer-basic-form [data-plugin='select2']").select2({dropdownAutoWidth: true});
        if (_visible == VISIBLE_PRIVATE) {
            ...
            $("#search-ownerUser").select2({dropdownAutoWidth: true});
        ...
        $("#search-basic-tag-dropdown-menu-container .dropdown-menu-checkbox-list, #search-adv-tag-dropdown-menu-container .dropdown-menu-checkbox-list").click(function (b) {
            b.stopPropagation()
        });
        $("#search-basic-tag-dropdown, #search-adv-tag-dropdown").on("shown.bs.dropdown", function () {
            $(this).find(".dropdown-menu-checkbox-list").asScrollable({contentSelector: ">", containerSelector: ">"})
        });
    ...
    addCustomer: function () {
        ...
        var arry = $("#form-add-customer").serialize();
        // arry = decodeURIComponent(arry,true);
        $.ajax({
            url: "/customer/add",
            type: "POST",
    ...
    loadCustomer: function (a) {
        ...
        $.ajax({
            ...
            type: "POST",
            data: JSON.stringify(b.searchParams),
            dataType: "json",
            timeout: 60000,

            error: function (e, c, d) {
                $("#search-loading").hide();
                alertify.alert("加载败，请稍后再试")
            },
            success: function (c) {
                $("#search-loading").hide();
                if (c.code <= 0) {
    ...
    showCustomerItem: function (a, i) {
        ...
        var j = $("#customer-item-clone").clone(true).removeClass("hide").addClass("list-item").attr("id", "customer-item-" + i.encodedId);
        var b = true;
        ...
        j.find(".item-ownerUser").text(i.encodedOwnerUserId);
        ...
        j.find(".item-nextContactDate").text(getFormatDate(i.nextContactDate, "YYYY-MM-DD HH:mm:ss"));
        j.find(".item-createDate").text(getFormatDate(i.createDate, "YYYY-MM-DD HH:mm:ss"));
        j.find(".item-createUser").text(i.encodedCreateUserId);
        j.find(".item-assignDate").text(getFormatDate(i.assignDate, "YYYY-MM-DD HH:mm:ss"));
        if (i.encodedLastContactUserId != "") {
            j.find(".item-lastContactUser").text(i.encodedLastContactUserId)
        }
        j.find(".item-lastContactDate").text(getFormatDate(i.lastContactDate, "YYYY-MM-DD HH:mm:ss"));
        if (a == -1) {
    ...
    changeCategory: function () {
        ...
        $.ajax({
            ...
            type: "POST",
            data: {ids: entity.selectEntityIds, categoryId: $("#customer-changeCategory").val()},
            dataType: "json",
            timeout: 60000,
            error: function (e, c, d) {
                a.button("reset");
    ...
    batchChangeTag: function () {
        ...
        $.ajax({
            ...
            type: "POST",
            data: {ids: entity.selectEntityIds, tagIds: e, type: b},
            dataType: "json",
            timeout: 60000,
        ...
        $.ajax({
            ...
            success: function (b) {
                ...
                $.each(b.items.notes, function (c, d) {
                    $("span[id='customer-item-ext-note-" + d.encodedId + "']").text(d.remark).parent().parent().removeClass("hide")
                })
            }
        })
    },
