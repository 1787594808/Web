var entity = {
    clickLink: "", selectEntityIds: "", loadEntityStatistics: function (a) {
        $.ajax({
        ...
        $.ajax({
            url: c, type: "POST", data: b, dataType: "json", timeout: 60000, error: function (i, g, h) {
                e.button("reset");
        ...
        $("[data-plugin='customModal']").click(function (b) {
            a.selectEntityIds = $("#entity-item-list input:checked[name=item-checkbox]").map(function () {
                return $(this).data("id")
            }).get().join();
        ...
        $.slidePanel.show({url: a, settings: {method: "GET"}}, {
            ...
            pointerDrag: false,
            template: function (b) {
                return '<div class="' + b.classes.base + " " + b.classes.base + "-" + b.direction + '"><div class="' + b.classes.base + '-scrollable"><div><div class="' + b.classes.content + '"></div></div></div><div class="' + b.classes.base + '-handler"></div></div>'
            },
            afterLoad: function (b, c) {
                $(".slidePanel-scrollable").asScrollable({contentSelector: ">", containerSelector: ">"})
            },
            loading: {
                template: function (b) {
        ...
        if (b && b.length > 0) {
            $.ajax({type: "POST", data: {customerId: b, name: "**"}, url: "/" + a + "/search/name"}).then(function (d) {
                if (d.code <= 0) {
                    return
                }
        ...
        return {
            ...
            ajax: {
                ...
                delay: 250,
                data: function (a) {
                    return {name: a.term, page: a.page}
        ...
        $("#search-dropdown").on("shown.bs.dropdown", function () {
            $(this).find(".dropdown-menu-list").asScrollable({contentSelector: ">", containerSelector: ">"})
        });
